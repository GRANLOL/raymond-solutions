...
function getUnavailableServiceMessage(reason) {
    ...
    if (reason === 'lunch_break') {
        return 'В это время действует обед. Выберите слот после перерыва или более раннее время.';
    }
    if (reason === 'custom_break') {
        return 'На это время настроен перерыв. Выберите другой слот.';
    }
    if (reason === 'crosses_break') {
        return 'Эта услуга не помещается до перерыва целиком. Выберите более ранний слот или время после перерыва.';
    }
    if (reason === 'past') {

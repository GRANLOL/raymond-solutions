...
async def webapp_edit_logo_data_cb(callback: types.CallbackQuery, state: FSMContext):
    ...
    if logo_type == "url":
        prompt = (
            "🖼 <b>Настройка картинки/логотипа</b>\n\n"
            "Вы можете просто <b>отправить фотографию</b> прямо в этот чат, и она автоматически станет логотипом вашего сервиса!\n\n"
            "<i>(Или отправьте ссылку на картинку, например `https://.../logo.png`)</i>\n\n"
            "Отправьте `-` чтобы очистить и вернуть стандартный логотип."
        )
    else:
        prompt = (
